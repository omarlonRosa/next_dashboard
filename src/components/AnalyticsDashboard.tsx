...
const Badge = ({percentage} : {percentage: number}) => {
    ...
    return (
        <span
            className={`inline-flex gap-1 items-center rounded-md px-2 py-1 text-xs font-semibold ring-1 ring-inset ${
                isPositive 
                    ? positiveClassname 
                    : isNeutral
                        ? neutralClassname 
    ...
    return(
        ...
        <Card className="w-full">
            <p className=" flex gap-2.5 items-center text-tremor-default text-dark-tremor-content">
                ...
                <Badge
                    percentage={
                        (amtVisitorsToday / Number(avgVisistorsPerDay) - 1) * 100
                    }
                />
            </p>
            <p className="text-3xl text-dark-tremor-content-strong font-semibold">
            ...
            <Card className="flex flex-col sm:grid grid-cols-4 gap-6">
                <h2 className="w-full text-dark-tremor-content-strong
                text-center sm:left-left font-semibold text-x1">
                    Principais visitantes desta semana
                </h2>
                ...
                justify-between flex-wrap gap-8">
                    {topCountries?.map(([countryCode, number]) => {
                        ...
                        text-dark-tremor-content-strong">
                            ...
                            sm:text-dark-tremor-content-strong">
                                {number}
                               </p>
                        </div>
                    )
